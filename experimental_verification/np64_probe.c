...
int main(int argc, char **argv) {
    ...
    int narr = 512;
    // 2D arrays to match TeaLeaf memory pressure
    ...
    // UNSERIALIZED (like CGT/TSC_NATIVE)
    ...
    // SERIALIZED (like TSC/PAPI, adding CPUID)
    for (int it = 0; it < ntest; it++) {
        for (uint64_t j=1; j<narr-1; j++) {
            uint64_t t0 = read_cgt();
    ...
    if (rank == 0) {
        ...
        printf("Unserialized Kernel (CGT/Wtime style): %lu ns/iter\n", global_unser / size);
        printf("Serialized Kernel (TSC/PAPI style):    %lu ns/iter\n", global_ser / size);
    }
