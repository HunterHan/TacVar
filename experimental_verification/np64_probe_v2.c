...
int main(int argc, char **argv) {
    ...
    // SERIALIZED (Using LFENCE instead of CPUID to match TSC_FENCE / TSC overhead more closely without massive CPUID exit overhead)
    for (int it = 0; it < ntest; it++) {
        for (uint64_t j=1; j<narr-1; j++) {
    ...
    if (rank == 0) {
        printf("--- NP=%d Memory Contention Test ---\n", size);
        printf("Unserialized Kernel (CGT style): %lu ns/iter\n", global_unser / size);
        printf("Serialized Kernel (LFENCE style):    %lu ns/iter\n", global_ser / size);
    }
