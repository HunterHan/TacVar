...
static pow_data_t *kern_data = NULL;

void init_kern_pow(size_t flush_kib) {
    if (kern_data == NULL) {
        kern_data = malloc(sizeof(pow_data_t));
        if (kern_data == NULL) {
            fprintf(stderr, "Failed to allocate memory for pow kernel\n");
            return;
        }
...
void run_kern_pow(void) {
    if (kern_data && kern_data->a && kern_data->b) {
        for (uint64_t i = 0; i < kern_data->npf; i++) {
            kern_data->a[i] = pow(kern_data->b[i], 1.0001);
        }
    }
}
