...
static copy_data_t *kern_data = NULL;

void init_kern_copy(size_t flush_kib) {
    if (kern_data == NULL) {
        kern_data = malloc(sizeof(copy_data_t));
        if (kern_data == NULL) {
            fprintf(stderr, "Failed to allocate memory for copy kernel\n");
            return;
        }
