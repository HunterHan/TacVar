...
calc_tr(prob_hist_t *tmh, prob_hist_t *trh, i64 *tf_arr, u64 tf_len, filt_param_t *args){
	...
	int err = 0;
	double tot_p = 0.0;
	double simh[tmh->nbin], simh2[tmh->nbin];
    double dp_min;
    const double ep_fill_threshold = 0.05;
    FILE *dbg_fp = fopen("calc_tr_residual.0608.csv", "w");

    if (dbg_fp != NULL) {
        fprintf(dbg_fp, "imb,tr_l,tr_r,tm_p,sim_p,residual,initial_p,dmdr,tr_p,tot_p,status\n");
    }

    dp_min = fabs(0.01 * args->p_low);

    i64 tf_anchor = tf_arr[tf_len / 2];
    printf("[FilT-calc_tr-v2] using tf median anchor=%lld instead of tf_min=%lld, tf_len=%llu\n",
           (long long)tf_anchor, (long long)tf_arr[0], (unsigned long long)tf_len);
	...
	if (trh->pbin == NULL) {
        ...
        if (dbg_fp != NULL) fclose(dbg_fp);
        err = errno;
        return err;
    }
	for (u64 i = 0; i < nbin; i ++) {
		trh->pbin[i].t = tmh->pbin[i].t - tf_anchor;
		trh->pbin[i].p = 0.0;
        simh[i] = 0;
        simh2[i] = 0;
	}

	// Loop over each time section in trh. This keeps the original greedy estimator,
    // but records residuals so failed bins are visible after the run.
    imb = 0;
	while (imb < nbin - 1) {
		i64 tr_l = trh->pbin[imb].t, tr_r = trh->pbin[imb+1].t;
        double p, tr_p = 0;
        double dp = 0, dmdr = 0, dp0, dp1;
        const char *status = "fit";

        p = tmh->pbin[imb].p - simh[imb];
        if (p < dp_min) {
            trh->pbin[imb].p = 0;
            status = "skip_low_residual";
            if (dbg_fp != NULL) {
                fprintf(dbg_fp, "%llu,%lld,%lld,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%s\n",
                        (unsigned long long)imb, tr_l, tr_r, tmh->pbin[imb].p, simh[imb],
                        p, p, dmdr, trh->pbin[imb].p, tot_p, status);
            }
            imb += 1;
            continue;
        }
        ...
        if (fabs(dmdr) <= 1e-12 || !isfinite(dmdr)) {
            tr_p = trh->pbin[imb].p;
            status = "keep_small_dmdr";
        ...
        if (tr_p < 0 || !isfinite(tr_p)) {
            tr_p = 0;
            status = "reset_invalid";
        }

        if (tr_p > 1.0 - tot_p && p > 0 && p <= 1.0 - tot_p) {
            tr_p = p;
            trh->pbin[imb].p = tr_p;
            sim_met(tmh, trh, tf_arr, tf_len, simh, args->nsamp);
            status = "keep_initial_overshoot";
        }

        if (tot_p + tr_p >= 1.0) {
            double remaining = 1.0 - tot_p;
            if (remaining < 0) {
                remaining = 0;
            }
            if (tr_p > remaining) {
                status = "cap_remaining_mass";
                tr_p = remaining;
            }
            trh->pbin[imb].p = tr_p;
            tot_p += tr_p;
            if (dbg_fp != NULL) {
                fprintf(dbg_fp, "%llu,%lld,%lld,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%s\n",
                        (unsigned long long)imb, tr_l, tr_r, tmh->pbin[imb].p, simh[imb],
                        tmh->pbin[imb].p - simh[imb], p, dmdr, tr_p, tot_p, status);
            }
            break;
        } else {
            tot_p += tr_p;
    ...
    // Conservative fallback: when the greedy estimator leaves too much probability
    // mass missing, distribute the missing mass according to positive residuals.
    // This avoids turning a failed low-mass solution into a misleading two-spike
    // distribution solely by normalization.
    if (tot_p > 0 && ep > ep_fill_threshold && tot_p < 1.0) {
        double missing = 1.0 - tot_p;
        double residual_sum = 0.0;
        sim_met(tmh, trh, tf_arr, tf_len, simh, args->nsamp);
        for (u64 i = 0; i < nbin - 1; i ++) {
            double residual = tmh->pbin[i].p - simh[i];
            if (residual > 0 && isfinite(residual)) {
                residual_sum += residual;
            }
        }
        printf("[FilT-calc_tr] Conservative residual fill: ep=%f, missing=%f, residual_sum=%f\n",
               ep, missing, residual_sum);
        if (residual_sum > 0) {
            for (u64 i = 0; i < nbin - 1; i ++) {
                double residual = tmh->pbin[i].p - simh[i];
                if (residual > 0 && isfinite(residual)) {
                    double add_p = missing * residual / residual_sum;
                    trh->pbin[i].p += add_p;
                    if (dbg_fp != NULL) {
                        fprintf(dbg_fp, "%llu,%lld,%lld,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%s\n",
                                (unsigned long long)i, trh->pbin[i].t, trh->pbin[i+1].t,
                                tmh->pbin[i].p, simh[i], residual, residual, 0.0,
                                trh->pbin[i].p, 1.0, "residual_fill");
                    }
                }
            }
            tot_p = 1.0;
