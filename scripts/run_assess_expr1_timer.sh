#!/bin/bash -e
# Assess ta-only variant generated from run_detecting_expr1_timer.sh
set -euo pipefail
...
EXPR_NAME="${EXPR_NAME:-assess.expr1.timer}"
      ...
      for rkern in ${RKERN_LIST}; do
        for fsize_kib in "${FSIZE_LIST[@]}"; do
          ...
          while IFS=, read -r walk_idx ta_ns tb_ns; do
            ...
            [[ "${VERBOSE_WALKS:-0}" == "1" ]] && echo "Completed walk ${idx}/${walk_count} for timer=${timer}, interval=${mu_ns} ns, fkern=${fkern}, rkern=${rkern}, fsize=${fsize_kib} KiB"
          done < "${WALK_LIST}"
        done
      done
    done
