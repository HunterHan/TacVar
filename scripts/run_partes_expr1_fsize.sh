...
# TIMER="${TIMER:-clock_gettime mpi_wtime tsc_asym papi papix6 likwid}"
# TIMER="${TIMER:-clock_gettime mpi_wtime papi papix6}"
TIMER="${TIMER:-clock_gettime mpi_wtime tsc_asym papi}"
...
for timer in ${TIMER_LIST}; do
  for fsize_kib in "${FSIZE_LIST[@]}"; do
    echo ""
    echo "=== Running all walks for timer=${timer}, fsize=${fsize_kib} KiB ==="
    idx=0
    while IFS=, read -r walk_idx ta_ns tb_ns; do
      ...
      tb_ns="${tb_ns%$'\r'}"
      idx=$((idx+1))
      run_one_walk "${walk_idx}" "${ta_ns}" "${tb_ns}" "${timer}" "${fsize_kib}"
      echo "Completed ${idx}/${walk_count} for timer=${timer}, fsize=${fsize_kib} KiB"
    done < "${WALK_LIST}"
  done
done
