...
# Run ParTES with per-walk (ta==tb) from a fixed-time walk list (shared at OUT_ROOT).
#   <timestamp>/walk_list_normal.csv, walk_list_meta.txt
#   <timestamp>/<timer>/<combo>/meta.md and <timer>_walks/...
...
# Space-separated interval list (ns) for combo paths; default single 10000.
INTERVAL_LIST="${INTERVAL_LIST:-10000}"
...
FKERN_LIST="${FKERN_LIST:-copy add scale triad pow}"
RKERN_LIST="${RKERN_LIST:-copy add scale triad pow}"
...
FSIZE_LIST=(4096)
RSIZE_LIST=(${RSIZE_LIST[@]:-4096})

# Walk list: one file at OUT_ROOT with --fixed-ns ${WALK_FIXED_NS} and NWALKS; all combos read it (see gen_walklist.py).
...
cp -f "${BASH_SOURCE[0]}" "${ARCHIVE_DIR}/run_detecting_expr3_frkern.sh"
