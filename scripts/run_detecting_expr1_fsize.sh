...
EXPR_NAME="detecting.expr1.fsize2"
        ...
        for fsize_kib in "${FSIZE_LIST[@]}"; do
          for rsize_kib in "${RSIZE_LIST[@]}"; do
            ...
            while IFS=, read -r walk_idx ta_ns tb_ns; do
              ...
              echo "Completed ${idx}/${walk_count} for timer=${timer}, interval=${mu_ns} ns, fkern=${fkern}, rkern=${rkern}, fsize=${fsize_kib} KiB, rsize=${rsize_kib} KiB"
            done < "${WALK_LIST}"
          done
        done
      done
