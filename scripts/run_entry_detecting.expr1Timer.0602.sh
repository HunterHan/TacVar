...
walk_default(){ echo "${WALK_ROOT}/detecting_expr1_timer_Normal_n${NUM_WALK}_tbase${TBASE_NS}.csv"; }
...
do_detect(){
    ...
    for np in ${NP_LIST}; do
        for timer in ${TIMER_LIST}; do
            combo="${data_folder}/${EXPR_NAME}/np${np}/${timer}/timer_${timer}_interval${TBASE_NS}"
            run_one "${combo}" "${walk_list}" "${np}" "${timer}" "${FKERN}" "${FSIZE_KIB}" "${RKERN}" "${RSIZE_KIB}" "${TBASE_NS}"
        done
    done
}
