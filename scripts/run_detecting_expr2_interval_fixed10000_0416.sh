...
# Run ParTES with per-walk (ta==tb) from a fixed-time walk list (see gen_walklist.py --dist fixed).
# Outputs are organized as:
#   scripts/output/<host>/<expr>/<timestamp>/
#     <timer>/<combo>/walk_list_normal.csv, meta.txt, meta.md
#     <timer>/<combo>/<timer>_walks/w0000_ta<ns>/partes_*.csv, run.log
EXPR_NAME="detecting.expr2.interval.fixed"
...
# Space-separated interval list (ns) for combo paths; sweep labels (interval*) may differ from the shared walk list.
# Default test space (as requested): 1e3, 1e4, 1e5, 1e6, 1e7.
INTERVAL_LIST="${INTERVAL_LIST:-1000 10000 100000 1000000 10000000}"
# Single OUT_ROOT walk list uses --fixed-ns WALK_FIXED_NS (default: first token of INTERVAL_LIST).
...
echo "[INFO] INTERVAL_LIST='${INTERVAL_LIST}'"
if [[ -z "${WALK_FIXED_NS}" ]]; then
  WALK_FIXED_NS="$(echo ${INTERVAL_LIST} | awk '{print $1}')"
fi
echo "[INFO] WALK_FIXED_NS='${WALK_FIXED_NS}' (single walk_list_normal.csv at OUT_ROOT)"
...
# Walk list: one file at OUT_ROOT generated with --fixed-ns ${WALK_FIXED_NS} and NWALKS; all combos read it (see gen_walklist.py).
...
cp -f "${BASH_SOURCE[0]}" "${ARCHIVE_DIR}/run_detecting_expr2_interval.sh"
...
_n_iv_words="$(echo ${INTERVAL_LIST} | wc -w | tr -d ' ')"
if [[ "${_n_iv_words}" -gt 1 ]]; then
  echo "[WARN] INTERVAL_LIST has multiple values; every combo still uses the same ${SHARED_WALK_LIST} with --fixed-ns ${WALK_FIXED_NS}. Override with env WALK_FIXED_NS=..."
fi
python3 "${SCRIPT_DIR}/../utils/gen_walklist.py" \
  --dist fixed \
  --nwalks "${NWALKS}" \
  --fixed-ns "${WALK_FIXED_NS}" \
  ...
  (
    ...
    mpirun --map-by core --bind-to core -np "${NP}" \
      ...
      "${BINARY}" \
        ...
        --rkern-b "${rkern}" --rsize-b "${rsize_kib}" \
...
for timer in ${TIMER_LIST}; do
  for interval_ns in ${INTERVAL_LIST}; do
    for fkern in ${FKERN_LIST}; do
      for rkern in ${RKERN_LIST}; do
        for fsize_kib in "${FSIZE_LIST[@]}"; do
          for rsize_kib in "${RSIZE_LIST[@]}"; do
          combo="interval${interval_ns}_fkern${fkern}_rkern${rkern}_fsize${fsize_kib}_rsize${rsize_kib}"
...

          {
            ...
            echo "| interval_list | \`${INTERVAL_LIST}\` |"
            echo "| walk_fixed_ns | \`${WALK_FIXED_NS}\` |"
            echo "| nwalks | \`${NWALKS}\` |"
            echo "| fkern | \`${fkern}\` |"
          ...
          echo ""
          echo "=== Running all walks for timer=${timer}, interval=${interval_ns} ns, fkern=${fkern}, rkern=${rkern}, fsize=${fsize_kib} KiB, rsize=${rsize_kib} KiB ==="
          idx=0
          while IFS=, read -r walk_idx ta_ns tb_ns; do
            ...
            tb_ns="${tb_ns%$'\r'}"
            idx=$((idx+1))
            run_one_walk "${walk_idx}" "${ta_ns}" "${tb_ns}" "${timer}" "${combo_root}" "${fkern}" "${rkern}" "${fsize_kib}" "${rsize_kib}" "${interval_ns}"
            echo "Completed ${idx}/${walk_count} for timer=${timer}, interval=${interval_ns} ns, fkern=${fkern}, rkern=${rkern}, fsize=${fsize_kib} KiB, rsize=${rsize_kib} KiB"
          done < "${WALK_LIST}"
          done
        done
      done
