#!/bin/bash -e
# Assess ta-only variant generated from run_detecting_expr2_interval.sh
set -euo pipefail
...
EXPR_NAME="${EXPR_NAME:-assess.expr2.interval}"
...
if [[ -n "${WALK_MU_NS}" ]]; then
  echo "[INFO] WALK_MU_NS='${WALK_MU_NS}' (override: all interval combos use this walk-list mean)"
else
  echo "[INFO] WALK_MU_NS unset: each interval combo generates its own walk list from its mu value."
fi
...
print_assess_overview() {
  ...
  echo "  nwalks per interval: ${NWALKS}"
  echo "  walk_distribution: N(0, sigma_rel=${SIGMA_REL}) + tbase"
  echo "  walklist_root: ${WALKLIST_ROOT}"
  echo ""
}
print_assess_overview
