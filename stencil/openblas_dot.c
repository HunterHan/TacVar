...
/*
 * Target kernel adapted from OpenBLAS kernel/generic/dot.c (double dot product form).
 *
...
__attribute__((noinline)) static double target_unit(double *A, double *B, double *C, double *D, uint64_t n, uint64_t row) {
    ...
    double *x = A + row * n;
    double *y = B + row * n;
    for (uint64_t i = 0; i < n; i++) dot += x[i] * y[i];
    return dot;
}
...
int main(int argc, char **argv) {
...
#endif
    if (myrank == 0) printf("OpenBLAS DOT target kernel. NTEST=%d, NPASS=%d, N=%lu\n", ntest, NPASS, narr);
    ...
    char fname[512]; sprintf(fname, "openblas_dot_time_%d_%s.csv", myrank, myhost);
