...
/*
 * C adaptation of NASA NPB 3.4.4 MPI FT routines fft_init, cfftz, fftz2:
 *   NPB3.4.4/NPB3.4-MPI/FT/ft.f90
 *   https://www.nas.nasa.gov/assets/npb/NPB3.4.4.tar.gz
 * Upstream ft.f90 SHA256:
 *   1847aa6136446e2727503a75964500cd7e2f70b1c4fe5299263dc6a768fe44fb
 * Stockham stage order and fftblock=16 are preserved. TacVar times one
 * fftz2 stage per sample. This omits the full FT transpose/evolve/reporting.
 */
...
static int ilog2_exact(uint64_t n) {
    int m = 0;
    uint64_t v = n;
    while (v > 1 && (v & 1) == 0) { v >>= 1; ++m; }
    return v == 1 ? m : -1;
}

static void fft_init(uint64_t n, double complex *u) {
    int m = ilog2_exact(n);
    uint64_t ku = 1, ln = 1;
    u[0] = (double)m;
    for (int j=1; j<=m; ++j) {
        double t = M_PI / (double)ln;
        for (uint64_t i=0; i<ln; ++i) {
            double ti = (double)i * t;
            u[ku+i] = cos(ti) + I*sin(ti);
        }
        ku += ln;
        ln *= 2;
    }
...
static void fftz2(int is, int l, int m, uint64_t n, int ny, int ny1,
                  const double complex *u, const double complex *x,
                  double complex *y) {
    uint64_t n1=n/2, lk=1ULL<<(l-1), li=1ULL<<(m-l), lj=2*lk, ku=li;
    for (uint64_t i=0; i<li; ++i) {
        uint64_t i11=i*lk, i12=i11+n1, i21=i*lj, i22=i21+lk;
        double complex u1 = is>=1 ? u[ku+i] : conj(u[ku+i]);
        for (uint64_t k=0; k<lk; ++k)
            for (int j=0; j<ny; ++j) {
                double complex x11=x[(i11+k)*ny1+j];
                double complex x21=x[(i12+k)*ny1+j];
...
static void cfftz(int is, int m, uint64_t n, const double complex *u,
                  double complex *x, double complex *y) {
    for (int l=1; l<=m; l+=2) {
        fftz2(is,l,m,n,FFTBLOCK,FFTBLOCK,u,x,y);
        if (l==m) { memcpy(x,y,(size_t)FFTBLOCK*n*sizeof(*x)); return; }
        fftz2(is,l+1,m,n,FFTBLOCK,FFTBLOCK,u,y,x);
    }
}
...
main(int argc, char **argv) {
    ...
    m = ilog2_exact(narr);
    if (m < 1) { fprintf(stderr, "FFT size must be a power of two\n"); return -1; }
    ...
    u = (double complex *)malloc((narr+1)*sizeof(*u));
    x = (double complex *)malloc((size_t)FFTBLOCK*narr*sizeof(*x));
    y = (double complex *)malloc((size_t)FFTBLOCK*narr*sizeof(*y));
    fft_init(narr,u);
    for (uint64_t k=0; k<narr; ++k)
        for (int j=0; j<FFTBLOCK; ++j) {
            x[k*FFTBLOCK+j]=(1.0+(k%17)*0.01)+I*(0.5+(j%7)*0.01);
            y[k*FFTBLOCK+j]=0.0;
        }
    ...
    if (myrank == 0) {
        printf("NASA NPB-FT Stockham FFT target kernel (not the complete benchmark).\nNTEST=%lu, NPASS=%u, N=%lu, FFTBLOCK=%d\n",
                ntest, NPASS, narr, FFTBLOCK);
    }
...
// #endif
    ...
    int64_t *p_ev;
    p_ev = (int64_t *)malloc(ntest * (size_t)m * nev * sizeof(int64_t));
    ...
    p_ns = (uint64_t *)malloc(ntest * (size_t)m * sizeof(uint64_t));
// #if defined(USE_PAPI) || defined(USE_TSC)
...
// #endif
    ...
    for (int it = 0; it < ntest; it ++) {
        for (int stage = 0; stage < m; stage ++) {
            ...
            if (l & 1) fftz2(1,l,m,narr,FFTBLOCK,FFTBLOCK,u,x,y);
            else       fftz2(1,l,m,narr,FFTBLOCK,FFTBLOCK,u,y,x);
#endif
...
// Timing.
            ...
            p_ns[it*m+stage] = (uint64_t)(ns1 - ns0);
            ...
            for (int iev = 0; iev < nev; iev ++) {
                p_ev[it * m * nev + stage * nev + iev] = (int64_t)(ev_vals_1[iev] - ev_vals_0[iev]);
            }
            p_ns[it*m+stage] = (uint64_t)(ns1 - ns0);
            ...
            p_ns[it*m+stage] = ns1 - ns0;

#elif USE_WTIME
            wtime1 = MPI_Wtime();
            cpuid_serialize();
            p_ns[it*m+stage] = (uint64_t)((wtime1 - wtime0) * 1e9);
            ...
            for (int iev = 0; iev < nev; iev ++) {
                p_ev[it * m * nev + stage * nev + iev] = (int64_t)ev_vals_1[iev] - (int64_t)ev_vals_0[iev];
            ...
            p_ns[it*m+stage] = (uint64_t)((double)(ns1 - ns0) * tsc_ns);
...
#ifdef USE_PAPI
    sprintf(fname, "npb_ft_fft_papi_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CGT
    sprintf(fname, "npb_ft_fft_cgt_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_WTIME
    sprintf(fname, "npb_ft_fft_wtime_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCT
    sprintf(fname, "npb_ft_fft_cntvct_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCTO
    sprintf(fname, "npb_ft_fft_cntvcto_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_PAPIX6
    sprintf(fname, "npb_ft_fft_papix6_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_LIKWID
    sprintf(fname, "npb_ft_fft_likwid_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_FENCE)
    sprintf(fname, "npb_ft_fft_tsc_fence_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_NATIVE)
    sprintf(fname, "npb_ft_fft_tsc_native_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC)
    sprintf(fname, "npb_ft_fft_tsc_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#else
    sprintf(fname, "npb_ft_fft_stiming_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#endif
    ...
    for (int it = NPASS; it < ntest; it ++) {
        for (int j = 0; j < m; j ++) {
#ifndef STAGE_TF
            fprintf(fp, "%d,%lu", myrank, p_ns[it*m+j]);
#else
            fprintf(fp, "%d,%lu,%lu", myrank, nsamp, p_ns[it*m+j]);
#endif

#if defined(USE_LIKWID) || defined(USE_PAPIX6)
            for (int iev = 0; iev < nev; iev ++) {
                fprintf(fp, ",%ld", p_ev[it*m*nev+j*nev+iev]);
            }
#endif
...
// #endif
    ...
    double checksum = 0.0;
    for (size_t i=0; i<(size_t)FFTBLOCK*narr; ++i)
        checksum += creal(x[i])+cimag(x[i])+creal(y[i])+cimag(y[i]);
    if (myrank == 0) {
