...
main(int argc, char **argv) {
    ...
    if (myrank == 0) {
        printf("PolyBench/C GEMM target kernel (not the complete benchmark).\nNTEST=%lu, NPASS=%u, N=%lu \n",
                ntest, NPASS, narr);
    }
...
#ifdef USE_PAPI
    sprintf(fname, "polybench_gemm_papi_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CGT
    sprintf(fname, "polybench_gemm_cgt_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_WTIME
    sprintf(fname, "polybench_gemm_wtime_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCT
    sprintf(fname, "polybench_gemm_cntvct_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCTO
    sprintf(fname, "polybench_gemm_cntvcto_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_PAPIX6
    sprintf(fname, "polybench_gemm_papix6_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_LIKWID
    sprintf(fname, "polybench_gemm_likwid_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_FENCE)
    sprintf(fname, "polybench_gemm_tsc_fence_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_NATIVE)
    sprintf(fname, "polybench_gemm_tsc_native_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC)
    sprintf(fname, "polybench_gemm_tsc_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#else
    sprintf(fname, "polybench_gemm_stiming_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#endif
