...
/*
 * Target kernel adapted from OpenBLAS kernel/generic/gemv_n.c (non-transposed GEMV row accumulation form).
 *
...
__attribute__((noinline)) static double target_unit(double *A, double *B, double *C, double *D, uint64_t n, uint64_t row) {
    ...
    double *a = A + row * n;
    double *x = B;
    for (uint64_t j = 0; j < n; j++) sum += a[j] * x[j];
    C[row] += alpha * sum;
    return C[row];
}
...
int main(int argc, char **argv) {
...
#endif
    if (myrank == 0) printf("OpenBLAS GEMV target kernel. NTEST=%d, NPASS=%d, N=%lu\n", ntest, NPASS, narr);
    ...
    char fname[512]; sprintf(fname, "openblas_gemv_time_%d_%s.csv", myrank, myhost);
