...
/*
 * Target kernel implementing the OpenBLAS AXPY interface semantics y := y + alpha*x as portable C, not an architecture-specific OpenBLAS microkernel.
 *
...
__attribute__((noinline)) static double target_unit(double *A, double *B, double *C, double *D, uint64_t n, uint64_t row) {
    (void)C; (void)D;
    const double alpha = 1.61803398875;
    double *x = A + row * n;
    double *y = B + row * n;
...
int main(int argc, char **argv) {
...
#endif
    if (myrank == 0) printf("OpenBLAS AXPY target kernel. NTEST=%d, NPASS=%d, N=%lu\n", ntest, NPASS, narr);
    ...
    char fname[512]; sprintf(fname, "openblas_axpy_time_%d_%s.csv", myrank, myhost);
