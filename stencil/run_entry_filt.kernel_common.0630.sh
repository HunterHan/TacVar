#!/bin/bash -x
# 0630 per-kernel full filtering runner. Invoke through run_entry_filt.<kernel>.0630.sh.
set -euo pipefail
...
: "${KERNEL_NAME:?KERNEL_NAME must be set by wrapper}"
KERNEL_EXT="${KERNEL_EXT:-c}"
OUTPUT_ROOT="${OUTPUT_ROOT:-output_${KERNEL_NAME}_filt_dsub}"
DATE_STAMP_DEFAULT="${DATE_STAMP_DEFAULT:-20260630-${KERNEL_NAME//_/-}-dsub}"
SIZE_LIST_DEFAULT="${SIZE_LIST_DEFAULT:-64 128 256 512 1024 2048}"
SMOKE_SIZE_DEFAULT="${SMOKE_SIZE_DEFAULT:-64}"
...
cleanup_residual_processes(){
    local pattern="${KERNEL_NAME}_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt\.${KERNEL_NAME}\.0630|run_entry_filt\.kernel_common\.0630"
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    local data_folder=$1
    mkdir -p "${data_folder}"
    local src="${STENCIL_ROOT}/${KERNEL_NAME}.${KERNEL_EXT}"
    {
        echo "script=$(realpath "$0")"
        echo "common_script=${SCRIPT_DIR}/run_entry_filt.kernel_common.0630.sh"
        ...
        echo "common_script_sha256=$(sha256sum "${SCRIPT_DIR}/run_entry_filt.kernel_common.0630.sh" 2>/dev/null | awk '{print $1}')"
        echo "kernel_sha256=$(sha256sum "$src" 2>/dev/null | awk '{print $1}')"
    ...
    cp -f "${SCRIPT_DIR}/run_entry_filt.kernel_common.0630.sh" "${data_folder}/run_entry_filt.kernel_common.0630.sh"
}
...
BASE_CFLAGS="$CFLAGS"
PREWARM=${PREWARM:-0}
if [ "$PREWARM" = "1" ]; then BASE_CFLAGS="$BASE_CFLAGS -DUSE_PREWARM"; fi
...
SHUFFLE_SEED=${SHUFFLE_SEED:-0630}
NP_LIST=${NP_LIST:-"64"}
SIZE_LIST=${SIZE_LIST:-${SIZE_LIST_DEFAULT}}

ARCH=$(uname -m)
HOSTNAME=$(hostname -s 2>/dev/null || hostname)
DATE_BASE=${DATE_BASE:-20260630}
DATE_STAMP=${DATE_STAMP:-${DATE_STAMP_DEFAULT}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/${OUTPUT_ROOT}/${DATE_STAMP}"
...
BASE_TIMER_LIST="${TIMER_LIST:-cgt papi papix6 wtime}"
case $ARCH in
    x86_64) TIMER_LIST="${BASE_TIMER_LIST} tsc" ;;
    aarch64) TIMER_LIST="${BASE_TIMER_LIST} cntvcto" ;;
    *) echo "Unsupported architecture: $ARCH"; exit 1 ;;
esac
if [ "${SMOKE:-0}" = "1" ]; then
    SHUFFLE_COUNT=1
    TIMER_LIST="cgt"
    SIZE_LIST="${SMOKE_SIZE_DEFAULT}"
    NP_LIST="64"
...
printf 'ARCH: %s\nHOSTNAME: %s\nDATA_FOLDER: %s\nKERNEL: %s\nNP_LIST: %s\nTIMER_LIST: %s\nSIZE_LIST: %s\nINSITU: %s\nINSITU_TAG: %s\nSHUFFLE_COUNT: %s\nSHUFFLE_SEED: %s\nCOMMIT_HASH: %s\nNSPV: %s\nTHEORETICAL_NSPV_FACTOR: %s\nEFFECTIVE_NSPV: %s\n' "$ARCH" "$HOSTNAME" "$DATA_FOLDER" "$KERNEL_NAME" "$NP_LIST" "$TIMER_LIST" "$SIZE_LIST" "$INSITU" "$INSITU_TAG" "$SHUFFLE_COUNT" "$SHUFFLE_SEED" "$COMMIT_HASH" "$NSPV" "$THEORETICAL_NSPV_FACTOR" "$EFFECTIVE_NSPV"
...
compile_one(){
    local timer=$1
    local mode=$2
    local out="${KERNEL_NAME}_${timer}${mode}.x"
    local src="${KERNEL_NAME}.${KERNEL_EXT}"
    local timer_cflags="$BASE_CFLAGS"
    case $timer in
    ...
    for timer in $shuffled_timers; do
        for np in $NP_LIST; do
            for size in $SIZE_LIST; do
                ...
                "${run_cmd[@]}" "./${KERNEL_NAME}_${timer}.x" "${size}" 0 1
                mv ./*.csv "$tm_dir/"
                for nsamp_ratio in ${NSAMP_RATIO_LIST}; do
                    ...
                    "${run_cmd[@]}" "./${KERNEL_NAME}_${timer}_tf.x" "${size}" 0 1 "${nsamp_tf}"
                    mv ./*.csv "$te_dir/"
