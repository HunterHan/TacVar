...
cleanup_residual_processes(){
    local pattern='jacobi2d5p_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt\.0629'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "jacobi_before_commit=${JACOBI_BEFORE_COMMIT}"
        echo "jacobi_before_sha256=${JACOBI_BEFORE_SHA256}"
        echo "jacobi_after_sha256=${JACOBI_AFTER_SHA256}"
...
BUILD_ROOT="${STENCIL_ROOT}/.build_0629_jacobi_ab"
...
VARIANT_LIST=${VARIANT_LIST:-"before after"}
NP_LIST=${NP_LIST:-"64"}
TIMER_LIST=${TIMER_LIST:-"cgt papi papix6 wtime"}
SIZE_LIST="1024"
JACOBI_BEFORE_COMMIT=${JACOBI_BEFORE_COMMIT:-aa7bc09}

ARCH=$(uname -m)
HOSTNAME=$(hostname -s 2>/dev/null || hostname)
DATE_BASE=${DATE_BASE:-20260629}
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-jacobi0629-ab-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_jacobi_timing_ab_0629_${INSITU_TAG}/${DATE_STAMP}"
...
mkdir -p "${BUILD_ROOT}"
if [ -n "${JACOBI_BEFORE_SRC:-}" ]; then
    cp -f "${JACOBI_BEFORE_SRC}" "${BUILD_ROOT}/jacobi2d5p_before.c"
else
    git -C "${PROJ_ROOT}" show "${JACOBI_BEFORE_COMMIT}:stencil/jacobi2d5p.c" > "${BUILD_ROOT}/jacobi2d5p_before.c"
fi
cp -f "${STENCIL_ROOT}/jacobi2d5p.c" "${BUILD_ROOT}/jacobi2d5p_after.c"
JACOBI_BEFORE_SHA256=$(sha256sum "${BUILD_ROOT}/jacobi2d5p_before.c" | awk '{print $1}')
JACOBI_AFTER_SHA256=$(sha256sum "${BUILD_ROOT}/jacobi2d5p_after.c" | awk '{print $1}')
...
printf 'ARCH: %s\nHOSTNAME: %s\nDATA_FOLDER: %s\nVARIANT_LIST: %s\nNP_LIST: %s\nTIMER_LIST: %s\nSIZE_LIST: %s\nINSITU: %s\nINSITU_TAG: %s\nSHUFFLE_COUNT: %s\nSHUFFLE_SEED: %s\nCOMMIT_HASH: %s\nJACOBI_BEFORE_COMMIT: %s\nJACOBI_BEFORE_SHA256: %s\nJACOBI_AFTER_SHA256: %s\nNSPV: %s\nTHEORETICAL_NSPV_FACTOR: %s\nEFFECTIVE_NSPV: %s\n' "$ARCH" "$HOSTNAME" "$DATA_FOLDER" "$VARIANT_LIST" "$NP_LIST" "$TIMER_LIST" "$SIZE_LIST" "$INSITU" "$INSITU_TAG" "$SHUFFLE_COUNT" "$SHUFFLE_SEED" "$COMMIT_HASH" "$JACOBI_BEFORE_COMMIT" "$JACOBI_BEFORE_SHA256" "$JACOBI_AFTER_SHA256" "$NSPV" "$THEORETICAL_NSPV_FACTOR" "$EFFECTIVE_NSPV"

mkdir -p "$DATA_FOLDER"
write_root_meta "$DATA_FOLDER"
cd "${STENCIL_ROOT}"
rm -f ./jacobi2d5p_*.x ./*.csv
mpicc -O2 -Wall -o "${FILTER_ROOT}/filt.x" "${FILTER_ROOT}/filt_v2.0608.c"

for variant in $VARIANT_LIST; do
    case "$variant" in
        before) src="${BUILD_ROOT}/jacobi2d5p_before.c" ;;
        after) src="${BUILD_ROOT}/jacobi2d5p_after.c" ;;
    ...
    for timer in $TIMER_LIST; do
        ...
        mpicc -o "jacobi2d5p_${variant}_${timer}.x" "$src" $timer_cflags -D"$INSITU" -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
        mpicc -o "jacobi2d5p_${variant}_${timer}_tf.x" "$src" $timer_cflags -D"$INSITU" -DSTAGE_TF -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
    done
done
        ...
        for timer in $shuffled_timers; do
            for np in $NP_LIST; do
                for size in $SIZE_LIST; do
                    tm_dir="${shuffle_dir}/jacobi2d5p_${timer}_np${np}_size${size}"
                    rm -rf "$tm_dir" ./*.csv
                    mkdir -p "$tm_dir"
                    ...
                    "${run_cmd[@]}" "./jacobi2d5p_${variant}_${timer}.x" "${size}" 0 1
                    mv ./*.csv "$tm_dir/"
                    for nsamp_ratio in ${NSAMP_RATIO_LIST}; do
                        ...
                        "${run_cmd[@]}" "./jacobi2d5p_${variant}_${timer}_tf.x" "${size}" 0 1 "${nsamp_tf}"
                        mv ./*.csv "$te_dir/"
