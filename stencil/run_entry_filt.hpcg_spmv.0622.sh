...
cleanup_residual_processes(){
    local pattern='hpcg_spmv_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "kernel_sha256=$(sha256sum "${STENCIL_ROOT}/hpcg_spmv.cpp" 2>/dev/null | awk '{print $1}')"
        echo "upstream=HPCG:114602d458d1034faa52b71e4c15aba9b3a17698:src/ComputeSPMV_ref.cpp"
...
SHUFFLE_SEED=0622
KERNEL_LIST="hpcg_spmv"
NP_LIST=${NP_LIST:-"64"}
TIMER_LIST=${TIMER_LIST:-"cgt papi papix6 wtime"}
SIZE_LIST=${SIZE_LIST:-"8 12 16 24 32 48 64"}
...
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-hpcg-spmv-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_hpcg_spmv_filt_${INSITU_TAG}/${DATE_STAMP}"
...
for kernel in $KERNEL_LIST; do
    for timer in $TIMER_LIST; do
        ...
        mpicxx -std=c++11 -o "${kernel}_${timer}.x" "${kernel}.cpp" $timer_cflags -D"$INSITU" -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
        mpicxx -std=c++11 -o "${kernel}_${timer}_tf.x" "${kernel}.cpp" $timer_cflags -D"$INSITU" -DSTAGE_TF -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
    done
done
