...
cleanup_residual_processes(){
    local pattern='polybench_gemm_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "kernel_sha256=$(sha256sum "${STENCIL_ROOT}/polybench_gemm.c" 2>/dev/null | awk '{print $1}')"
        echo "upstream=PolyBench/C-4.2.1-beta:linear-algebra/blas/gemm/gemm.c"
...
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-polybench-gemm-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_polybench_gemm_filt_${INSITU_TAG}/${DATE_STAMP}"
...
for kernel in $KERNEL_LIST; do
    for timer in $TIMER_LIST; do
        ...
        mpicc -std=c11 -o "${kernel}_${timer}.x" "${kernel}.c" $timer_cflags -D"$INSITU" -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
        mpicc -std=c11 -o "${kernel}_${timer}_tf.x" "${kernel}.c" $timer_cflags -D"$INSITU" -DSTAGE_TF -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
    done
done
