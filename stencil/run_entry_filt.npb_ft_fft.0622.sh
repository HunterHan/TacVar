...
cleanup_residual_processes(){
    local pattern='npb_ft_fft_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "kernel_sha256=$(sha256sum "${STENCIL_ROOT}/npb_ft_fft.c" 2>/dev/null | awk '{print $1}')"
        echo "upstream=NASA-NPB-3.4.4:NPB3.4-MPI/FT/ft.f90"
...
SHUFFLE_SEED=0622
KERNEL_LIST="npb_ft_fft"
NP_LIST=${NP_LIST:-"64"}
TIMER_LIST=${TIMER_LIST:-"cgt papi papix6 wtime"}
SIZE_LIST=${SIZE_LIST:-"64 128 256 512 1024 2048 4096"}
...
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-npb-ft-fft-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_npb_ft_fft_filt_${INSITU_TAG}/${DATE_STAMP}"
...
for kernel in $KERNEL_LIST; do
    for timer in $TIMER_LIST; do
        ...
        mpicc -std=c11 -o "${kernel}_${timer}.x" "${kernel}.c" $timer_cflags -D"$INSITU" -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib" -lm
        mpicc -std=c11 -o "${kernel}_${timer}_tf.x" "${kernel}.c" $timer_cflags -D"$INSITU" -DSTAGE_TF -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib" -lm
    done
done
