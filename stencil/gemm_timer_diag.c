...
/*
 * Target kernel adapted from PolyBench/C 4.2.1-beta:
 *   linear-algebra/blas/gemm/gemm.c
 *   https://sourceforge.net/projects/polybench/files/polybench-c-4.2.1-beta.tar.gz
 * Upstream gemm.c SHA256:
 *   1568eec3636bbca13d576643eb9d44b280fb37409679a747ae89123e5f310acc
 *
 * The initialization and i-k-j kernel order follow upstream.  TacVar adds
 * MPI/timer/filter plumbing and times one complete outer i iteration.  This
 * is a target-kernel experiment, not the complete PolyBench benchmark.
 */
...
#endif
#define GEMM_DIAG_STR2(x) #x
#define GEMM_DIAG_STR(x) GEMM_DIAG_STR2(x)

#if defined(USE_TSC_CURRENT)
#define USE_TSC 1
#endif
#if defined(USE_TSC_NATIVE_CURRENT)
#define USE_TSC_NATIVE 1
#endif
#if defined(USE_TSC_PRE_CGT)
#define USE_TSC 1
#endif
#if defined(USE_PAPI_TIME_ONLY)
#define USE_PAPI 1
#endif
#if defined(USE_PAPIX6_CURRENT) || defined(USE_PAPIX6_READ_BEFORE_NS0) || defined(USE_PAPIX6_NO_READ)
#define USE_PAPIX6 1
#endif

#ifdef TIMING
#if defined(USE_PAPI) || defined(USE_PAPIX6) || defined(SANITY_ALL)
#include "papi.h"
...
#elif defined(USE_TSC_LFENCE_MEM)
    uint32_t lo, hi;
    __asm__ volatile ("lfence\n\trdtsc\n\t" : "=a"(lo), "=d"(hi) :: "memory");
    *cycle = ((uint64_t)hi << 32) | lo;
...
main(int argc, char **argv) {
...
#ifdef SANITY_ALL
    if (argc >= 2 && strcmp(argv[1], "--sanity") == 0) {
        ...
        double local_tsc_ns = calibrate_ns_per_tsc();
        if (myrank == 0) {
            printf("Sanity calibrated TSC frequency: %f GHz\n", 1e0 / local_tsc_ns);
        }
        int eventset = PAPI_NULL;
        PAPI_library_init(PAPI_VER_CURRENT);
        PAPI_create_eventset(&eventset);
        PAPI_start(eventset);
        char myhost[1024], fname[4096];
        gethostname(myhost, 1024);
        sprintf(fname, "gemm_timer_diag_sanity_%d_%s.csv", myrank, myhost);
        FILE *fp = fopen(fname, "w");
        fprintf(fp, "rank,duration_ms,timer,elapsed_ns\n");
        int durs_ms[2] = {10, 100};
        for (int idur = 0; idur < 2; idur++) {
            struct timespec req = {.tv_sec = 0, .tv_nsec = durs_ms[idur] * 1000000L};
            uint64_t t0, t1;
            struct timespec a, b;
            double w0, w1;
            long long p0, p1;
            tsc_start(&t0); nanosleep(&req, NULL); tsc_stop(&t1);
            fprintf(fp, "%d,%d,tsc_current,%lu\n", myrank, durs_ms[idur], (uint64_t)((double)(t1 - t0) * local_tsc_ns));
            clock_gettime(CLOCK_MONOTONIC, &a); nanosleep(&req, NULL); clock_gettime(CLOCK_MONOTONIC, &b);
            fprintf(fp, "%d,%d,cgt_current,%lu\n", myrank, durs_ms[idur], (uint64_t)((b.tv_sec-a.tv_sec)*1000000000ull + (uint64_t)(b.tv_nsec-a.tv_nsec)));
            w0 = MPI_Wtime(); nanosleep(&req, NULL); w1 = MPI_Wtime();
            fprintf(fp, "%d,%d,wtime_current,%lu\n", myrank, durs_ms[idur], (uint64_t)((w1 - w0) * 1e9));
            p0 = PAPI_get_real_nsec(); nanosleep(&req, NULL); p1 = PAPI_get_real_nsec();
            fprintf(fp, "%d,%d,papi_time_only,%lu\n", myrank, durs_ms[idur], (uint64_t)(p1 - p0));
        }
        fclose(fp);
        PAPI_shutdown();
        MPI_Finalize();
    ...
    for (size_t i = 0; i < narr; i ++) {
        C[i] = (double *)malloc(narr * sizeof(double));
    }
    for (size_t i = 0; i < narr; i ++) {
        A[i] = (double *)malloc(narr * sizeof(double));
    }
    for (size_t i = 0; i < narr; i ++) {
        B[i] = (double *)malloc(narr * sizeof(double));
    }
    ...
    if (myrank == 0) {
        printf("PolyBench/C GEMM timer diagnostic target kernel (not the complete benchmark).\nNTEST=%lu, NPASS=%u, N=%lu \n",
                ntest, NPASS, narr);
    }
    ...
    do {
        for (uint64_t i = 0; i < narr; i ++) {
            for (uint64_t j = 0; j < narr; j ++) {
                C[i][j] = (double)((i * j + 1) % narr) / narr;
                A[i][j] = (double)(i * (j + 1) % narr) / narr;
                B[i][j] = (double)(i * (j + 2) % narr) / narr;
            }
        }
        if (myrank == 0) {
...
#elif USE_PAPIX6
#if defined(USE_PAPIX6_READ_BEFORE_NS0)
            PAPI_read(eventset, ev_vals_0);
            ns0 = PAPI_get_real_nsec();
#elif defined(USE_PAPIX6_NO_READ)
            ns0 = PAPI_get_real_nsec();
#else
            ns0 = PAPI_get_real_nsec();
            PAPI_read(eventset, ev_vals_0);
...
#elif defined(USE_TSC) || defined(USE_TSC_FENCE) || defined(USE_TSC_NATIVE) || defined(USE_TSC_CPUID_MEM) || defined(USE_TSC_LFENCE_MEM)
#if defined(USE_TSC_PRE_CGT)
            { struct timespec tv_dummy; clock_gettime(CLOCK_MONOTONIC, &tv_dummy); }
#endif
            tsc_start(&ns0);
            // cycle0 = ns0;
...
// Timing.
            ...
            for (int iev = 0; iev < nev; iev ++) {
                p_ev[it * narr * nev + i * nev + iev] = (int64_t)ev_vals_1[iev] - (int64_t)ev_vals_0[iev];
            ...
            p_ns[it*narr+i] = (uint64_t)((double)(ns1 - ns0) * tsc_ns);
    ...
    gethostname(myhost, 1024);
sprintf(fname, "gemm_timer_diag_%s_time_%d_%s.csv", GEMM_DIAG_STR(TIMER_NAME), myrank, myhost);
    FILE *fp = fopen(fname, "w");
