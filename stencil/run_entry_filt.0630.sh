#!/bin/bash -x
# 0630 Jacobi-after full filtering runner. Fixed to jacobi2d5p only; excludes tsc_native and cntvct.
set -euo pipefail
...
cleanup_residual_processes(){
    local pattern='jacobi2d5p_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt\.0630'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "jacobi_source_sha256=$(sha256sum "${STENCIL_ROOT}/jacobi2d5p.c" 2>/dev/null | awk '{print $1}')"
...
DATE_STAMP=${DATE_STAMP:-20260630-jacobi-after-full-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_jacobi_after_full_0630_${INSITU_TAG}/${DATE_STAMP}"
...
case $ARCH in
    x86_64) TIMER_LIST="$TIMER_LIST tsc" ;;
    aarch64) TIMER_LIST="$TIMER_LIST cntvcto" ;;
