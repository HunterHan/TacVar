...
cleanup_residual_processes(){
    local pattern='stream_triad_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt'
    local skip_pids=" $$ "
    local pid=$$
    local ppid
    while :; do
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "kernel_sha256=$(sha256sum "${STENCIL_ROOT}/stream_triad.c" 2>/dev/null | awk '{print $1}')"
...
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-stream-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_stream_filt_${INSITU_TAG}/${DATE_STAMP}"
