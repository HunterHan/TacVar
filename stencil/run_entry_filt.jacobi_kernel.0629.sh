...
BUILD_ROOT="${STENCIL_ROOT}/.build_0629_jacobi_kernel_ab"
...
initialize(){
    ...
    [ -r /sys/devices/system/cpu/intel_pstate/no_turbo ] && echo "intel_no_turbo=$(cat /sys/devices/system/cpu/intel_pstate/no_turbo)" || true
    [ -r /sys/devices/system/cpu/cpufreq/boost ] && echo "cpufreq_boost=$(cat /sys/devices/system/cpu/cpufreq/boost)" || true
}
...
cleanup_residual_processes(){
    local pattern='jacobi2d5p_.*\.x|jacobi2d5p_kernel_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt\.jacobi_kernel\.0629'
    local skip_pids=" $$ " pid=$$ ppid
    while :; do
...
shuffle_timer_list(){
    local shuffle_idx=$1; shift
    "${PYTHON}" - "${SHUFFLE_SEED}" "${shuffle_idx}" "$@" <<'PY_SHUFFLE'
import random, sys
seed=sys.argv[1]; shuffle_idx=int(sys.argv[2]); items=sys.argv[3:]
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "git_status_begin"; git -C "${PROJ_ROOT}" status --short || true; echo "git_status_end"
        echo "host=${HOSTNAME}"
        echo "arch=${ARCH}"
        ...
        echo "kernel_list=jacobi2d5p jacobi2d5p_kernel"
        ...
        echo "row_ntest=source_default"
        echo "kernel_ntest=${KERNEL_NTEST}"
        echo "row_source_sha256=${ROW_SOURCE_SHA256}"
...
write_shuffle_meta(){
    local shuffle_dir=$1 shuffle_id=$2 shuffled_timers=$3
    mkdir -p "${shuffle_dir}"
    { cat "${DATA_FOLDER}/meta.txt"; echo "shuffle_id=${shuffle_id}"; echo "shuffle_timer_list=${shuffled_timers}"; } > "${shuffle_dir}/meta.txt"
}
...
finish_run(){
    local status=$? end_epoch end_iso
    end_epoch=$(date +%s); end_iso=$(date -Is)
...
SHUFFLE_SEED=${SHUFFLE_SEED:-0629}
VARIANT_LIST="row_after kernel_after"
NP_LIST="64"
TIMER_LIST=${TIMER_LIST:-"cgt papi papix6 wtime"}
SIZE_LIST="1024"
KERNEL_NTEST=${KERNEL_NTEST:-1000}
if [ "${SMOKE:-0}" = "1" ]; then
    SHUFFLE_COUNT=1
    TIMER_LIST="cgt"
    NSAMP=20
    KERNEL_NTEST=${KERNEL_NTEST_SMOKE:-20}
fi

ARCH=$(uname -m)
HOSTNAME=$(hostname -s 2>/dev/null || hostname)
DATE_BASE=${DATE_BASE:-20260629}
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-jacobi-row-vs-kernel-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_jacobi_kernel_timing_0629_${INSITU_TAG}/${DATE_STAMP}"
COMMIT_HASH=${COMMIT_HASH:-$(git -C "${PROJ_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)}
case $HOSTNAME in
    cgnr6760pn2) CPU_FREQ=2.2 ;;
    *) echo "This runner is intentionally scoped to cgnr6760pn2; got ${HOSTNAME}"; exit 1 ;;
esac
: "${PAPI_HOME:?PAPI_HOME is not set}"
: "${OPENBLAS_HOME:?OPENBLAS_HOME is not set}"
if [ "$ARCH" = "x86_64" ]; then : "${LIKWID_HOME:?LIKWID_HOME is not set}"; fi
case $ARCH in
...
mkdir -p "${BUILD_ROOT}"
cp -f "${STENCIL_ROOT}/jacobi2d5p.c" "${BUILD_ROOT}/jacobi2d5p_row_after.c"
cp -f "${STENCIL_ROOT}/jacobi2d5p_kernel.c" "${BUILD_ROOT}/jacobi2d5p_kernel_after.c"
ROW_SOURCE_SHA256=$(sha256sum "${BUILD_ROOT}/jacobi2d5p_row_after.c" | awk '{print $1}')
KERNEL_SOURCE_SHA256=$(sha256sum "${BUILD_ROOT}/jacobi2d5p_kernel_after.c" | awk '{print $1}')
...
printf 'ARCH: %s\nHOSTNAME: %s\nDATA_FOLDER: %s\nVARIANT_LIST: %s\nNP_LIST: %s\nTIMER_LIST: %s\nSIZE_LIST: %s\nKERNEL_NTEST: %s\nINSITU: %s\nCOMMIT_HASH: %s\n' "$ARCH" "$HOSTNAME" "$DATA_FOLDER" "$VARIANT_LIST" "$NP_LIST" "$TIMER_LIST" "$SIZE_LIST" "$KERNEL_NTEST" "$INSITU" "$COMMIT_HASH"
mkdir -p "$DATA_FOLDER"
write_root_meta "$DATA_FOLDER"
cd "${STENCIL_ROOT}"
rm -f ./jacobi2d5p_*.x ./jacobi2d5p_kernel_*.x ./*.csv
mpicc -O2 -Wall -o "${FILTER_ROOT}/filt.x" "${FILTER_ROOT}/filt_v2.0608.c"

compile_one(){
    local variant=$1 timer=$2 src=$3 bin_prefix=$4 extra_cflags=$5
    local timer_cflags="$CFLAGS $extra_cflags"
    case $timer in
    ...
    mpicc -o "${bin_prefix}_${variant}_${timer}.x" "$src" $timer_cflags -D"$INSITU" -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
    mpicc -o "${bin_prefix}_${variant}_${timer}_tf.x" "$src" $timer_cflags -D"$INSITU" -DSTAGE_TF -DTIMING "-DUSE_${timer^^}" -I"${OPENBLAS_HOME}/include" -L"${OPENBLAS_HOME}/lib"
}
for timer in $TIMER_LIST; do
    compile_one row_after "$timer" "${BUILD_ROOT}/jacobi2d5p_row_after.c" "jacobi2d5p" ""
    compile_one kernel_after "$timer" "${BUILD_ROOT}/jacobi2d5p_kernel_after.c" "jacobi2d5p_kernel" "-DNTEST=${KERNEL_NTEST}"
done

for variant in $VARIANT_LIST; do
    case "$variant" in
        row_after) kernel_name="jacobi2d5p"; bin_prefix="jacobi2d5p" ;;
        kernel_after) kernel_name="jacobi2d5p_kernel"; bin_prefix="jacobi2d5p_kernel" ;;
        *) echo "Unsupported variant: $variant"; exit 1 ;;
    esac
    for shuffle_idx in $(seq 0 $((SHUFFLE_COUNT - 1))); do
        shuffle_id="shuffle${shuffle_idx}"
        shuffle_dir="${DATA_FOLDER}/${variant}/${shuffle_id}"
        shuffled_timers="$(shuffle_timer_list "${shuffle_idx}" ${TIMER_LIST})"
        write_shuffle_meta "${shuffle_dir}" "${shuffle_id}" "${shuffled_timers}"
        echo "${variant}_${shuffle_id}_timer_list=${shuffled_timers}" | tee -a "${DATA_FOLDER}/shuffle.log"
        for timer in $shuffled_timers; do
            for np in $NP_LIST; do
                for size in $SIZE_LIST; do
                    tm_dir="${shuffle_dir}/${kernel_name}_${timer}_np${np}_size${size}"
                    rm -rf "$tm_dir" ./*.csv
                    mkdir -p "$tm_dir"
                    run_cmd=(mpirun -np "$np" --map-by core --bind-to core)
                    "${run_cmd[@]}" "./${bin_prefix}_${variant}_${timer}.x" "${size}" 0 1
                    mv ./*.csv "$tm_dir/"
                    for nsamp_ratio in ${NSAMP_RATIO_LIST}; do
                        ...
                        "${run_cmd[@]}" "./${bin_prefix}_${variant}_${timer}_tf.x" "${size}" 0 1 "${nsamp_tf}"
                        mv ./*.csv "$te_dir/"
