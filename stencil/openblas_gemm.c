...
/*
 * Target kernel implementing OpenBLAS GEMM update semantics C := beta*C + alpha*A*B as portable C row kernel, not an architecture-specific packed OpenBLAS microkernel.
 *
...
__attribute__((noinline)) static double target_unit(double *A, double *B, double *C, double *D, uint64_t n, uint64_t row) {
    ...
    double *crow = C + row * n;
    for (uint64_t j = 0; j < n; j++) crow[j] *= beta;
    for (uint64_t k = 0; k < n; k++) {
        double aik = A[row * n + k];
        double *brow = B + k * n;
        for (uint64_t j = 0; j < n; j++) crow[j] += alpha * aik * brow[j];
    }
    return crow[(row * 13 + 7) % n];
}
...
int main(int argc, char **argv) {
...
#endif
    if (myrank == 0) printf("OpenBLAS GEMM target kernel. NTEST=%d, NPASS=%d, N=%lu\n", ntest, NPASS, narr);
    ...
    char fname[512]; sprintf(fname, "openblas_gemm_time_%d_%s.csv", myrank, myhost);
