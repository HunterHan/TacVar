...
cleanup_residual_processes(){
    local pattern='jacobi2d5p_.*\.x|jacobi2d5p_kernel_.*\.x|filt\.x|mpirun|prterun|orted|prted|run_entry_filt\.jacobi_kernel\.sizes64_256\.0630'
...
write_root_meta(){
    ...
    mkdir -p "${data_folder}"
    {
        ...
        echo "kernel_list=jacobi2d5p_kernel"
...
DATE_STAMP=${DATE_STAMP:-$(date +%Y%m%d-%H%M%S)-jacobi-kernel-sizes64-256-${INSITU_TAG}}
DATA_FOLDER="${DATA_ROOT}/${DATE_BASE}/${HOSTNAME}/output_jacobi_kernel_sizes64_256_0630_${INSITU_TAG}/${DATE_STAMP}"
