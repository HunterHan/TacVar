...
/*
 * Target kernel derived from the STREAM Triad computational form:
 *     A[i] = B[i] + scalar * C[i]
 *
 * This program reuses TacVar's MPI/timer/filter harness.  It is a target
 * kernel smoke test, not an implementation of the complete STREAM benchmark.
 */
...
main(int argc, char **argv) {
    uint64_t ntest;
    /* A, B, and C use the existing size-by-size sample layout. */
    double **a, **b, **c_arr;
    ...
    c_arr = (double **)malloc(narr * sizeof(double*));
    ...
    for (size_t i = 0; i < narr; i ++) {
        c_arr[i] = (double *)malloc(narr * sizeof(double));
    }
    ...
    if (myrank == 0) {
        printf("STREAM Triad target kernel (not the complete STREAM benchmark).\nNTEST=%lu, NPASS=%u, NARR=%lu \n",
                ntest, NPASS, narr);
    }
    ...
    do {
        for (uint64_t i = 0; i < narr; i ++) {
            fill_random(a[i], narr);
            fill_random(b[i], narr);
            fill_random(c_arr[i], narr);
        }
        if (myrank == 0) {
...
#ifdef USE_PAPI
    sprintf(fname, "stream_triad_papi_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CGT
    sprintf(fname, "stream_triad_cgt_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_WTIME
    sprintf(fname, "stream_triad_wtime_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCT
    sprintf(fname, "stream_triad_cntvct_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_CNTVCTO
    sprintf(fname, "stream_triad_cntvcto_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_PAPIX6
    sprintf(fname, "stream_triad_papix6_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif USE_LIKWID
    sprintf(fname, "stream_triad_likwid_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_FENCE)
    sprintf(fname, "stream_triad_tsc_fence_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC_NATIVE)
    sprintf(fname, "stream_triad_tsc_native_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#elif defined(USE_TSC)
    sprintf(fname, "stream_triad_tsc_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#else
    sprintf(fname, "stream_triad_stiming_time_%d_%s.csv", myrank, myhost);
    FILE *fp = fopen(fname, "w");
#endif
