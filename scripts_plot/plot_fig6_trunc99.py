...
def plot_expr(df: pd.DataFrame, expr: str, out_dir: Path, date: str) -> Path:
    ...
    fig.suptitle(f"{expr}: $R_L$ and $R_H$ vs fsize (top 1% truncated)", y=0.995)
    fig.tight_layout()
    out = out_dir / f"{expr}_rl_rh_trunc99_{date}.png"
...
def main() -> None:
    ...
    ap.add_argument("--output", type=Path, default=Path("scripts_plot/outputAssessing/fig6_np64_np128_final_trunc99"))
    args = ap.parse_args()
    ...
    summary = args.output / f"fig6_final_summary_trunc99_{args.date}.csv"
